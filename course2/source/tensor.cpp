...
void Tensor<float>::Padding(const std::vector<uint32_t>& pads,
  ...
  new_data.subcube(pad_rows1, pad_cols1, 0, pad_rows1+ this->data_.n_rows, pad_cols1+this->data_.n_cols, this->data_.n_slices) = this->data_;
  this->data_ = std::move(new_data);
  this->raw_shapes_ = {this->rows(), this->cols(), this->channels()};
}
...
void Tensor<float>::Flatten(bool row_major) {
  ...
  this->Reshape({origin_size}, row_major);
}
